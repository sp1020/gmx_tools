...
class RTPHandler:
    def __init__(self, cgenff_dir, output_ff_dir, rtp_filename, pdb_filename, resn='UNK'):
        ...
        if os.path.exists(self.output_ff_dir):
            ...
            print(
                '       Please remove it or choose a different directory before running the script.')
            sys.exit(1)
        shutil.copytree(self.cgenff_dir + '/charmm36.ff',
                        self.output_ff_dir + '/charmm36.ff')
                ...
                # Evaluate contents
                    ...
                    if current_section == 'bondedtypes':
                        ...
                        if handler:
                            ...
                            logging.debug(
                                f"Unkonwn section [ {current_section} ]")
    ...
    def modify_rtp(self, filepath):
        ...
        for e in tb_del['name']:
            if e in tb_mc['name']:
                print(
                    f'Error: atom in rename_mc sheet also in deletion group: {e}')
                flag_err = True
            if e in tb_aa['name']:
                print(
                    f'Error: atom in rename_aa sheet also in deletion group: {e}')
                flag_err = True
            if e in tb_others['name']:
                print(
                    f'Error: atom in rename_others sheet also in deletion group: {e}')
                flag_err = True

        # check whether rename instruction is not provided
        an_mod = set(tb_mc['name']).union(set(tb_aa['name'])).union(
            set(tb_others['name'])).union(set(tb_del['name']))
        ...
        intra_dup, inter_dup = self.check_overlaps(list(tb_mc['modified_name']), list(
            tb_aa['modified_name']), list(tb_others['modified_name']))
        ...
        print('Total charge of atoms in the original RTP file: %.3f' %
              charge_original_rtp)
        ...
        print('Total charge of atoms in the modified topology: %.3f' %
              charge_modification)
    ...
    def check_overlaps(self, *lists: List[Any]) -> Tuple[Dict[int, Set[Any]], Dict[Any, int]]:
        ...
        inter_counts = {item: freq for item,
                        freq in appearance.items() if freq > 1}
    ...
    def modify_forcefield_itp(self):
        lines = ''
        with open(self.output_ff_dir+'/charmm36.ff/forcefield.itp', 'r') as fin:
            for line in fin:
                lines += line
        with open(self.output_ff_dir+'/charmm36.ff/forcefield.itp', 'w') as fout:
            fout.write(lines)
            fout.write(
                '\n; modified by unnatural_aminoacid_ff_builder_cgenff\n')
            fout.write(f'#include "{self.resn_ori}_ffbonded.itp"')
...
def main():
    ...
    r = RTPHandler(args.cgenff_dir, args.output_dir,
                   args.rtp_file, args.pdb_file, resn=args.resn)
    r.modify_rtp(args.mod_file)
    r.write_rtp()
    r.convert_pdb()
    r.generate_residuetypes_file()
    r.modify_forcefield_itp()
